...
func storeDraftThread(iSvc string, iUpdt *Update) bool {
   ...
   if aId.tid() == "" {
      iUpdt.Thread.Cc = _updateCc(iSvc, iUpdt.Thread.Cc, false)
      var aSd *os.File
      aSd, err = os.Open(aDraft)
      if err != nil {
         if !os.IsNotExist(err) { quit(err) }
         ...
         aIdxN = 0
         aSd.Close()
         aNewSubjCc = len(iUpdt.Thread.Cc) != len(aCc)
      }
      aCc = iUpdt.Thread.Cc
   } else {
      aFd, err = os.OpenFile(aOrig, os.O_RDWR, 0600)
   ...
   if aIdxN == -1 {
      ...
      aNewSubjCc = aNewSubjCc || aEl.Subject != aIdx[aIdxN].Subject
...
type tMsgHead struct {
   Id string
   Len int64 `json:",omitempty"` //todo drop eventually
   Size int64
   Posted string
   From string
   SubHead tHeader2
...
func _readMsgHead(iFd *os.File) *tMsgHead {
   ...
   if err != nil { quit(err) }
   if aHead.Size == 0 && aHead.Len > 0 { // .Size added in 0.8
      aHead.Size = aHead.Len
      fmt.Fprintf(os.Stderr, "## _readMsgHead: in %s on %s, msgid %s lacks .Size\n",
                             iFd.Name(), aHead.Posted, aHead.Id)
   }
   return &aHead
}
...
func _writeMsg(iTd *os.File, iHead *Header, iR io.Reader, iEl *tIndexEl) (*tMsgHead, error) {
   ...
   aHead := tMsgHead{Id:iHead.Id, From:iHead.From, Posted:iHead.Posted, Size:aSize, SubHead:*iHead.SubHead}
